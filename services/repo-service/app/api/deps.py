"""
Shared FastAPI dependencies for the repo-service.

Provides:
  - get_db         — SQLModel session (one per request)
  - get_efs        — EFSService singleton
...
# ---------------------------------------------------------------------------
# EFS singleton — one per process, shared across requests
# ---------------------------------------------------------------------------

_efs = EFSService(settings.EFS_DRAFTS_ROOT)


def get_efs() -> EFSService:
    """FastAPI dependency — returns the process-level EFS service."""
    return _efs
